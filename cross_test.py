...
def test():
    with tf.Graph().as_default() as g:
        ...
        y_ = tf.placeholder(tf.float32, [None, cross_forward.OUTPUT_NODE])
        y = cross_forward.forward(x, None)

        ema = tf.train.ExponentialMovingAverage(cross_backward.MOVING_AVERAGE_DECAY)
        ...
        img_batch, label_batch = cross_generateds.get_tfrecord(TEST_NUM, isTrain=False)#2
